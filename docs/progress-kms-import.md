...
- **Outstanding manual smoke (Phase 3):** the real-KMS import acceptance criterion (import a throwaway RSA-2048 key into an EXTERNAL-origin KMS key, confirm `Enabled`) could **not** be run in the sandbox (no AWS credentials/network policy). The wire format is covered by KATs + a round-trip unit test, but the live import remains an operational check to perform before relying on the tool in production.
